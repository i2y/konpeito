...
| LLVM | 20 | CRuby ネイティブバックエンド |
| Java | 21+ | JVM バックエンド |

LLVM と Java は使うバックエンドに応じて必要になります。どちらか一方だけでも動きます。
...
`-I` なしでは `counter.rb` だけがコンパイルされ（約50 KB）、kumiki は実行時に CRuby がロードします。GUI アプリは I/O バウンドなので、実用上の差はほぼありません。
...
konpeito lsp                         # LSP サーバー起動（IDE 統合）
